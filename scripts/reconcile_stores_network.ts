...
async function finalPolish() {
    ...
    for (const s of lojas) {
        ...
        if (seen.has(upperName)) {
            ...
            await supabase.from('lojas').delete().eq('id', s.id);
        } else {
            if (s.name !== upperName) {
                console.log(`✨ Normalizando: ${s.name} -> ${upperName}`);
                await supabase.from('lojas').update({ name: upperName }).eq('id', s.id);
            }
            seen.add(upperName);
        }
    }

    const { data: final } = await supabase.from('lojas').select('name').order('name');
