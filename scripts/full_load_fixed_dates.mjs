...
async function loadFull() {
    ...
    console.log(`Iniciando importação de ${baseOficial.length} registros com datas corrigidas e métricas normalizadas...`);
    ...
    for (const row of baseOficial) {
        ...
        if (typeof row['DATA'] === 'number') {
            ...
            if (row['DATA'].includes('/')) {
                ...
                dataRef = row['DATA'].replace(/^0026/, '2026').substring(0, 10);
            }
        }
    ...
    console.log('Carga histórica com datas corrigidas completa.');
}
loadFull();
