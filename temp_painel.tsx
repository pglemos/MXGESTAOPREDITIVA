...
export default function PainelConsultor() {
    ...
    const fetchNetworkSnapshot = useCallback(async (isManual = false) => {
        ...
        try {
            ...
            // Buscando todos os dados da view sem filtros de tempo para garantir renderização de dados reais
            ...
            for (const row of aggregatedData || []) {
                ...
                console.log('DEBUG: Store ID processando:', sid);
            ...
            console.log('DEBUG: Lista de lojas disponíveis:', stores.map(s => s.id));
            ...
            // Buscando lojas diretamente para garantir a exibição
            const { data: allStores } = await supabase.from('stores').select('id, name');
            
            // Diagnóstico forcado baseado em todas as lojas encontradas no banco
            const diagnosticsMap: Record<string, StoreDiagnostic> = {}
            for (const store of allStores || []) {
                const sid = store.id;
                const s = salesMap[sid] || { total: 0, leads: 0, agd: 0, vis: 0 };
    ...
    const filteredAndSortedStores = useMemo(() => {
        ...
        console.log('DEBUG: Diagnostic state length:', result.length);

        // Retornando resultados sem filtro para garantir exibição
        return result
    }, [diagnostics, sortConfig])
