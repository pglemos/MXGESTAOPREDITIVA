    ...
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        {loading ? (
          ...
          <div className="space-y-6 pb-8">
            ...
            <div className="flex items-start gap-3 pt-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 mt-0.5">{cliente.canal_origem} · Cadastrado {moment(cliente.created_date).format("DD/MM/YYYY")}</p>
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  <span className="text-[11px] text-slate-500">{cliente.momento}</span>
                </div>
              </div>
              <button onClick={() => setEditando(e => !e)} className="p-2 rounded-xl hover:bg-slate-100 transition-colors">
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4">
              <p className="text-[11px] font-black text-[#005BFF] uppercase tracking-wide mb-1">Próxima Ação</p>
              <p className="text-sm font-semibold text-[#031B3D]">{cliente.proxima_acao || calcularProximaAcao(cliente)}</p>
            </div>
            ...
            <div className="flex gap-2">
              {waUrl && (
                <a href={waUrl} target="_blank" rel="noopener noreferrer" className="flex-1">
                  <Button className="w-full rounded-xl bg-[#005BFF] hover:bg-blue-700 text-white gap-2 text-sm">
            ...
            {editando ? (
              <div className="space-y-3 bg-slate-50 rounded-2xl p-4">
                <p className="text-xs font-black text-slate-400 uppercase tracking-wider">Editar dados</p>
                {[{ k: "veiculo_interesse", l: "Veículo de Interesse" }, { k: "email", l: "E-mail" }, { k: "valor_negociado", l: "Valor Negociado" }].map(({ k, l }) => (
                  <div key={k}>
                    <label className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1 block">{l}</label>
                    <Input value={form[k] || ""} onChange={e => setForm(p => ({ ...p, [k]: e.target.value }))} className="rounded-xl" />
                  </div>
                ))}
                <div>
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1 block">Temperatura</label>
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1 block">Momento do Cliente</label>
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1 block">Observações</label>
                ...
                {form.momento === "Visita agendada" && (
                  <div>
                    <label className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1 block">Visita Agendada</label>
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <Button onClick={salvarEdicao} className="flex-1 rounded-xl bg-[#005BFF] hover:bg-blue-700 text-white" disabled={salvando}>
              ...
              <div className="bg-slate-50 rounded-2xl p-4 space-y-4">
                <p className="text-xs font-black text-slate-400 uppercase tracking-wider">Informações</p>
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {cliente.proposta_enviada && <span className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full">Proposta enviada</span>}
                  {cliente.interesse_troca && <span className="text-xs bg-amber-50 text-amber-600 px-2 py-0.5 rounded-full">Interesse em troca</span>}
                  {cliente.interesse_financiamento && <span className="text-xs bg-purple-50 text-purple-600 px-2 py-0.5 rounded-full">Interesse em financiamento</span>}
                </div>
                {cliente.observacoes && (
                  <div>
                    <p className="text-[11px] text-slate-400 font-semibold uppercase tracking-wide">Observações</p>
                    <p className="text-sm text-slate-600 mt-0.5">{cliente.observacoes}</p>
                  </div>
                )}
                {cliente.motivo_perda && (
                  <div className="bg-red-50 rounded-xl p-3">
                    <p className="text-[11px] text-red-400 font-semibold uppercase tracking-wide">Motivo de Perda</p>
                    <p className="text-sm text-red-600 mt-0.5">{cliente.motivo_perda}</p>
                  </div>
                )}
              </div>
            )}
            ...
            {historico.length > 0 && (
              <div className="space-y-2">
                <p className="text-xs font-black text-slate-400 uppercase tracking-wider">Histórico</p>
                <div className="space-y-2">
                  {historico.map(h => (
                    <div key={h.id} className="flex items-start gap-2.5 py-2 border-b border-slate-50 last:border-0">
                      <div className="w-1.5 h-1.5 rounded-full bg-[#005BFF] mt-1.5 shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-slate-600">{h.tipo}</p>
                        <p className="text-xs text-slate-400">{h.descricao}</p>
                        {h.momento_novo && h.momento_novo !== h.momento_anterior && (
                          <p className="text-xs text-[#005BFF] mt-0.5">{h.momento_anterior} → {h.momento_novo}</p>
                        )}
                      </div>
          ...
          <div className="flex items-center justify-center h-48 text-slate-400">Cliente não encontrado.</div>
        )}
      </SheetContent>
    </Sheet>
  );
