...
function CardMissao({ missao, clientes, onClick }) {
  ...
  return (
    <button onClick={() => count > 0 && onClick(missao)}
      className={`text-left p-4 rounded-2xl border bg-white transition-all ${count > 0 ? "hover:border-[#005BFF] hover:shadow-sm cursor-pointer" : "opacity-50 cursor-not-allowed"} border-slate-100`}>
      ...
      <p className="text-xs text-slate-400 mt-1">{missao.objetivo}</p>
      <div className="flex items-center justify-between mt-3">
        <div className="flex items-center gap-1.5">
          <Users className="w-3.5 h-3.5 text-slate-400" />
          <span className="text-xs font-bold text-slate-600">{count} cliente{count !== 1 ? "s" : ""}</span>
        </div>
        <span className="text-xs text-[#005BFF] font-semibold">{missao.potencial}</span>
      </div>
    </button>
  );
}
  ...
  return (
    <div className="space-y-6">
          ...
          {clientesMissao.map(c => (
            <div key={c.id} className="bg-white border border-slate-100 rounded-xl px-4 py-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400">{c.momento}</p>
              </div>
              <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${c.temperatura === "Quente" ? "bg-red-50 text-red-600" : c.temperatura === "Morno" ? "bg-amber-50 text-amber-600" : "bg-slate-100 text-slate-500"}`}>
                {c.temperatura}
              </span>
            </div>
          ))}
      ...
      <Button onClick={() => onIniciar(missao, clientesMissao)}
        ...
        className="w-full rounded-xl bg-[#005BFF] hover:bg-blue-700 text-white gap-2 h-12 text-base font-bold">
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Escolha uma missão e trabalhe grupos de clientes com alto potencial de venda.</p>
      </div>

      {/* Mensagem personalizada */}
      <div className="bg-gradient-to-r from-[#005BFF] to-blue-600 rounded-2xl p-5 text-white">
        <p className="text-base font-bold leading-snug">
          Olá, {nomeVendedor}! Hoje existem <span className="text-blue-200 font-black">{totalOpcs} oportunidades</span> na sua carteira.
        </p>
        <p className="text-sm text-blue-100 mt-1">Comece pelas que possuem maior probabilidade de virar venda.</p>
