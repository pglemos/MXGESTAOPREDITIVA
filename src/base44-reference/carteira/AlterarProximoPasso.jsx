...
const OPCOES_PROXIMO_PASSO = [
  "Ligar para qualificar",
  "Enviar WhatsApp",
  "Definir forma de pagamento",
  "Confirmar orçamento",
  "Entender se possui troca",
  "Convidar para visita",
  "Confirmar visita",
  "Remarcar contato",
  "Retomar proposta",
  "Apresentar veículo recém-chegado",
  "Recuperar oportunidade",
  "Acionar gerente",
  "Revisar condição de financiamento",
  "Resolver avaliação do usado",
...
// ─── OBJETIVO AUTOMÁTICO ──────────────────────────────────────────────────────
const OBJETIVO_MAP = {
  "Ligar para qualificar":             "Entender interesse, pagamento e urgência.",
  "Enviar WhatsApp":                   "Retomar contato.",
  "Definir forma de pagamento":        "Avançar qualificação da compra.",
  "Confirmar orçamento":               "Validar condição de compra.",
  "Entender se possui troca":          "Identificar necessidade de avaliação.",
  "Convidar para visita":              "Gerar visita.",
  "Confirmar visita":                  "Proteger comparecimento.",
  "Remarcar contato":                  "Manter oportunidade ativa.",
  "Retomar proposta":                  "Avançar para decisão.",
  "Apresentar veículo recém-chegado":  "Gerar nova negociação.",
  "Recuperar oportunidade":            "Retomar engajamento.",
  "Acionar gerente":                   "Destravar negociação.",
  "Revisar condição de financiamento": "Resolver objeção de parcela.",
  "Resolver avaliação do usado":       "Destravar negociação com troca.",
  "Pedir sinal de negócio":            "Gerar compromisso.",
  "Aguardar retorno":                  "Acompanhar decisão do cliente.",
};

// ─── MOTIVO AUTOMÁTICO ────────────────────────────────────────────────────────
const MOTIVO_MAP = {
  "Confirmar visita":                  "Cliente possui visita agendada e precisa de confirmação.",
  "Retomar proposta":                  "Cliente recebeu proposta e ainda não teve evolução registrada.",
  "Recuperar oportunidade":            "Oportunidade perdeu velocidade e precisa ser retomada.",
  "Apresentar veículo recém-chegado":  "Entrou um veículo compatível com o interesse deste cliente.",
  "Acionar gerente":                   "A negociação possui uma objeção que exige apoio gerencial.",
  "Pedir sinal de negócio":            "Cliente demonstra decisão e pode avançar para compromisso.",
};

function getMotivoAuto(passo) {
  return MOTIVO_MAP[passo] || "Próximo passo ajustado pelo vendedor.";
}

// ─── MAPEAMENTO PENDÊNCIA → SUGESTÃO ─────────────────────────────────────────
const PENDENCIA_PARA_SUGESTAO = {
  "Definir forma de pagamento":   "Definir forma de pagamento",
  "Confirmar orçamento":          "Confirmar orçamento",
  "Entender se possui troca":     "Entender se possui troca",
  "Agendar visita":               "Convidar para visita",
  "Confirmar visita":             "Confirmar visita",
  "Retomar proposta":             "Retomar proposta",
  "Resolver objeção de preço":    "Acionar gerente",
  "Resolver avaliação do usado":  "Resolver avaliação do usado",
  "Recuperar contato":            "Recuperar oportunidade",
  "Registrar próximo passo":      null, // sem sugestão específica
};

// ─── MOTIVOS PARA "ACIONAR GERENTE" ──────────────────────────────────────────
const MOTIVOS_GERENTE = ["Preço", "Avaliação do usado", "Parcela", "Aprovação", "Outro"];
...
export default function AlterarProximoPasso({ open, onClose, cliente, pendencias = [], onSalvo }) {
  ...
  const [motivoGerente, setMotivoGerente] = useState("");
  ...
  useEffect(() => {
    if (open) {
      ...
      // Mapeia pendência → opção do modal
      const PENDENCIA_PARA_OPCAO = {
        "Definir forma de pagamento":        "Definir forma de pagamento",
        "Confirmar orçamento":               "Confirmar orçamento",
        "Entender se possui troca":          "Entender se possui troca",
        "Agendar visita":                    "Convidar para visita",
        ...
        "Revisar condição de financiamento": "Revisar condição de financiamento",
      };
      const passoInicial = prefill ? (PENDENCIA_PARA_OPCAO[prefill] || "") : "";
      setPasso(passoInicial);
      setDataStr(hoje);
      setHorario("");
      setObjetivo(passoInicial && passoInicial !== "Outro" ? (OBJETIVO_MAP[passoInicial] || "") : "");
      setObservacao("");
      setMotivoGerente("");
    }
  }, [open]);
  ...
  async function salvar() {
    ...
    // Adiciona motivo do gerente se aplicável
    const obsGerente = passo === "Acionar gerente" && motivoGerente ? `[Gerente - ${motivoGerente}] ${observacao}`.trim() : observacao;
  ...
  return (
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="max-w-md rounded-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-slate-50 rounded-xl px-3 py-2.5 space-y-0.5">
          ...
          {cliente.proximo_passo && (
            <p className="text-xs text-slate-500 mt-1">
              <span className="font-semibold">Atual:</span> {cliente.proximo_passo}
            </p>
          )}
        </div>
        ...
        {/* Seleção de próximo passo */}
        <div>
          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-2 block">Novo próximo passo *</label>
          <div className="grid grid-cols-2 gap-1.5 max-h-52 overflow-y-auto pr-1">
            {OPCOES_PROXIMO_PASSO.map(op => (
              <button
                key={op}
                onClick={() => setPasso(op)}
        ...
        {/* Acionar gerente — motivo */}
        {passo === "Acionar gerente" && (
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-2 block">Motivo para acionar gerente</label>
            <div className="flex flex-wrap gap-1.5">
              {MOTIVOS_GERENTE.map(m => (
                <button
                  key={m}
                  onClick={() => setMotivoGerente(m)}
                  className={`text-xs font-semibold px-3 py-1.5 rounded-xl border transition-all ${
                    motivoGerente === m
