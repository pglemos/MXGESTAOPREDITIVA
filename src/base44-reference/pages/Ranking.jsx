    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl p-12 shadow-sm border border-slate-100 text-center">
        ...
        <div className="mt-8 flex justify-center gap-4">
          {[1, 2, 3].map(pos => (
            <div key={pos} className={`w-20 rounded-2xl p-4 text-center ${pos === 1 ? "bg-mx-amber-light" : "bg-slate-50"}`}>
              <div className={`w-10 h-10 rounded-full mx-auto mb-2 flex items-center justify-center ${pos === 1 ? "bg-mx-amber text-white" : "bg-slate-200 text-slate-400"}`}>
                <Lock className="w-4 h-4" />
              </div>
              <p className="text-xs font-semibold text-slate-400">#{pos}</p>
            </div>
          ))}
        </div>
      </div>
