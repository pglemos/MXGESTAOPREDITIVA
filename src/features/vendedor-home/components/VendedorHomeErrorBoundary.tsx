...
/**
 * ErrorBoundary local do VendedorHome — usado por seção, para que falha
 * em uma section não derrube a página inteira (Story 3.4 reconciliada, ADR-0050).
 * Espelha o pattern do FeedbackErrorBoundary (Story 2.4) e DashboardErrorBoundary (Story 2.5).
 */
export class VendedorHomeErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    if (import.meta.env.DEV) {
      console.error(
        `[VendedorHomeErrorBoundary:${this.props.sectionName ?? 'unknown'}]`,
        error,
        info,
      )
    }
...
export default VendedorHomeErrorBoundary
