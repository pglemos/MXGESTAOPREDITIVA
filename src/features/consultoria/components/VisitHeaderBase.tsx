      ...
      <div className="relative z-10">
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-mx-md mb-mx-lg border-b border-border-subtle pb-mx-md">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-mx-md w-full lg:w-auto">
            ...
            <div className="space-y-mx-xs">
              ...
              <Input aria-label="Data" id="header-date" name="header-date" type="date" value={data.visit_date} onChange={e => onChange({ visit_date: e.target.value })} className="h-mx-10 bg-surface-alt/50 border border-border-subtle focus:bg-white focus:border-brand-primary font-bold transition-all shadow-sm" />
            </div>
            <div className="space-y-mx-xs">
              <Typography variant="tiny" tone="muted" className="uppercase font-bold">Tempo</Typography>
              <Input aria-label="Tempo" id="header-time" name="header-time" value={data.tempo} onChange={e => onChange({ tempo: e.target.value })} className="h-mx-10 bg-surface-alt/50 border border-border-subtle focus:bg-white focus:border-brand-primary font-bold transition-all shadow-sm" />
            </div>
            <div className="space-y-mx-xs">
              <Typography variant="tiny" tone="muted" className="uppercase font-bold">Alvo</Typography>
              <Input aria-label="Alvo" id="header-target" name="header-target" value={data.alvo} onChange={e => onChange({ alvo: e.target.value })} className="h-mx-10 bg-surface-alt/50 border border-border-subtle focus:bg-white focus:border-brand-primary font-bold transition-all shadow-sm" />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-mx-md">
          ...
          <div className="space-y-mx-xs p-mx-md bg-brand-secondary/5 rounded-mx-xl border border-brand-secondary/10 shadow-mx-sm">
            ...
            <Input aria-label="Projeção" id="header-projection" name="header-projection" value={data.projecao} onChange={e => onChange({ projecao: e.target.value })} className="bg-transparent border-none p-mx-0 text-2xl font-black text-brand-secondary focus-visible:ring-0" />
          </div>
          <div className="space-y-mx-xs p-mx-md bg-surface-alt rounded-mx-xl border border-border-default shadow-mx-sm">
            ...
            <Input aria-label="Leads (Mês)" id="header-leads" name="header-leads" value={data.leads_mes} onChange={e => onChange({ leads_mes: e.target.value })} className="bg-transparent border-none p-mx-0 text-2xl font-black text-text-primary focus-visible:ring-0" />
          </div>
          <div className="space-y-mx-xs p-mx-md bg-surface-alt rounded-mx-xl border border-border-default shadow-mx-sm">
            ...
            <Input aria-label="Estoque" id="header-stock" name="header-stock" value={data.estoque_disponivel} onChange={e => onChange({ estoque_disponivel: e.target.value })} className="bg-transparent border-none p-mx-0 text-2xl font-black text-text-primary focus-visible:ring-0" />
          </div>
        </div>
      </div>
    </Card>
