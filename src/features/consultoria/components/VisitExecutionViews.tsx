...
export function VisitFourExecution({ storeId, onGenerateSummary }: { storeId: string, onGenerateSummary: (t: string) => void }) { 
  ...
  return (
    <Card className="p-mx-lg shadow-mx-md border border-border-default bg-white rounded-2xl">
      <div className="flex items-center gap-mx-sm mb-6">
      ...
      <div className="space-y-mx-md">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-mx-lg">
          <div className="space-y-mx-md">
            ...
            <select value={v} onChange={e => setV(e.target.value)} className="w-full h-mx-10 px-3 rounded-lg border border-border-default bg-white text-sm font-bold">
          ...
          <div className="space-y-mx-md">
             ...
             <Textarea value={p} onChange={e => setP(e.target.value)} className="min-h-[80px] text-sm" placeholder="Pontos Fortes observados..." />
             <Textarea value={a} onChange={e => setA(e.target.value)} className="min-h-[80px] text-sm" placeholder="Ação de correção (O que vai mudar?)" />
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-mx-lg">
        <div className="space-y-mx-md">
          ...
          <div className="space-y-1">
            ...
            <Textarea value={participants} onChange={e => setParticipants(e.target.value)} placeholder="Quem participou?" className="min-h-[80px]" />
          </div>
        </div>
        <div className="space-y-mx-md">
          <div className="space-y-1">
            <Typography variant="tiny" className="font-bold text-text-tertiary uppercase">Parecer do Consultor</Typography>
            <Textarea value={observations} onChange={e => setObservations(e.target.value)} placeholder="Como foi o engajamento? Algum destaque?" className="min-h-[145px]" />
          </div>
        </div>
      </div>
...
export function VisitSixExecution({ clientId, clientSlug, onGenerateSummary }: { clientId: string, clientSlug: string, onGenerateSummary: (t: string) => void }) {
  ...
  return (
    <Card className="p-mx-lg shadow-mx-md border border-border-default bg-white rounded-2xl">
      ...
      <div className="space-y-1 mb-mx-md">
        <Typography variant="tiny" className="font-bold text-text-tertiary uppercase">Justificativa e Plano de Ataque</Typography>
        <Textarea value={rationale} onChange={e => setRationale(e.target.value)} placeholder="O que faremos para bater a nova meta?" className="min-h-[80px]" />
      </div>
    ...
    <Card className="p-mx-lg shadow-mx-md border border-border-default bg-white rounded-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-mx-lg">
        <div className="space-y-mx-md">
           ...
           <div className="space-y-2">
             {[
               ...
               <label key={i.k} className="flex items-center gap-3 p-3 bg-surface-alt/30 rounded-xl cursor-pointer hover:bg-surface-alt transition-colors">
                 ...
                 <Typography variant="p" className="text-xs font-bold text-text-secondary">{i.l}</Typography>
               </label>
             ))}
           </div>
        </div>
        ...
        <div className="space-y-mx-md">
          ...
          <Textarea value={o} onChange={e => setO(e.target.value)} className="min-h-[100px] text-sm" placeholder="Onde este vendedor quer estar em 6 meses?" />
        </div>
      </div>
