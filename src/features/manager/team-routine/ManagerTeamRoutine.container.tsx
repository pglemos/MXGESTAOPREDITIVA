...
export default function ManagerTeamRoutine() {
  ...
  const [historyActions, setHistoryActions] = useState<ActionRow[]>([])
  ...
  const fetchRoutine = useCallback(async () => {
    ...
    if (actionsResult.error || appointmentsResult.error) {
      ...
      setActions([]); setHistoryActions([]); setAppointments([])
    } else {
      const loadedActions = (actionsResult.data || []) as ActionRow[]
      setHistoryActions(loadedActions)
  ...
  const rows = useMemo(() => sellers.map(seller => {
    ...
    const execution = percent(metrics.completed, metrics.planned)
    const generatedAppointments = appointments.filter(item => item.seller_user_id === seller.id).length
    return { seller, ...metrics, execution, appointments: generatedAppointments, status: classifyRoutine(execution) }
  ...
  const trend = useMemo(() => buildRoutineTrend(historyActions, historyStart, date), [historyActions, historyStart, date])
...
function RoutineComparisonCard({ team, network, top }: { team: number; network: number | null; top: number | null }) {
  return <Card className="border border-border-subtle bg-white p-mx-lg shadow-mx-sm">
