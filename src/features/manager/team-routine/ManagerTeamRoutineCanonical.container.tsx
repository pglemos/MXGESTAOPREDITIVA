...
export default function ManagerTeamRoutineCanonical() {
  ...
  return <main className="min-h-full bg-gray-50"><div className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-4 py-6 pb-24">
    ...
    {chargeSellerId && <ManagerRoutineChargeModal open sellerName={rows.find(row => row.seller.id === chargeSellerId)?.seller.name || 'Vendedor'} date={format(parseISO(date), 'dd/MM/yyyy')} onClose={() => setChargeSellerId(null)} onSave={(message) => remind(chargeSellerId, message)}/>} 
  </div></main>
}
