...
/**
 * ErrorBoundary local do AgendaAdmin — usado por seção, para que falha
 * em uma section não derrube a página inteira (Story 2.6, ADR-0050).
 * Espelha o pattern do DashboardErrorBoundary (Story 2.5) e LandingErrorBoundary (Story 2.1).
 */
export class AgendaErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    if (import.meta.env.DEV) {
      console.error(`[AgendaErrorBoundary:${this.props.sectionName ?? 'unknown'}]`, error, info)
    }
  }
      ...
      return (
        ...
        >
          ...
          <span className="mt-mx-xs block normal-case tracking-normal text-text-secondary">
            O restante da agenda continua disponível. Recarregue para tentar novamente.
          </span>
        </div>
      )
    }
...
export default AgendaErrorBoundary
