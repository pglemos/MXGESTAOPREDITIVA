        ...
        <main className="w-full h-full flex flex-col gap-mx-lg p-mx-lg overflow-y-auto no-scrollbar bg-surface-alt">
            ...
            <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-mx-lg border-b border-border-default pb-10 shrink-0">
                <div className="flex flex-col gap-1">
                    ...
                    <Typography variant="caption" className="pl-mx-md uppercase tracking-widest font-black">PERSONAL DEVELOPMENT PLAN (PDI) • MX ACADEMY</Typography>
                </div>
                            ...
                            <header className="p-8 md:p-10 border-b border-border-default flex flex-col gap-8 sticky top-0 bg-white z-10">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <Typography variant="h2" className="uppercase tracking-tighter">Novo Plano de Evolução</Typography>
                                            <Typography variant="caption" tone="muted" className="font-black uppercase">Horizonte de Carreira & Competências</Typography>
                                        </div>
                                    </div>
                                    <Button variant="ghost" size="icon" onClick={() => setShowForm(false)} className="rounded-full w-12 h-12 hover:bg-surface-alt bg-white"><X size={24} /></Button>
                                </div>
                                    ...
                                    {steps.map((step, idx) => (
                                        <div key={step.id} className="flex-1 flex flex-col gap-2">
                                            ...
                                            <div className="flex items-center gap-2 px-2">
                                                ...
                                                <Typography variant="tiny" tone={idx <= currentStep ? 'default' : 'muted'} className="font-black">
                                                    {step.label}
                                                </Typography>
                                            </div>
                                        </div>
                                            ...
                                            {competences.map(c => (
                                                <div key={c.id} className="space-y-2">
                                                    <div className="flex justify-between items-center px-2">
                                                        <Typography variant="tiny" className="font-black uppercase tracking-widest">{c.label}</Typography>
                                                        <Typography variant="mono" tone="brand" className="text-xs font-black">{form[c.id as keyof typeof form]}</Typography>
                                                    </div>
                                                    <input 
                                                        type="range" min="1" max="10" 
                                                        value={form[c.id as keyof typeof form]}
                                                        onChange={e => setForm(f => ({ ...f, [c.id]: Number(e.target.value) }))}
                                                        className="w-full h-2 bg-surface-alt rounded-full appearance-none cursor-pointer accent-brand-primary shadow-inner"
                                                    />
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-mx-lg">
                                            <div className="space-y-8">
                                                ...
                                                {[1, 2, 3, 4, 5].map(num => (
                                                    <div key={num} className="relative group">
                                                        <div className="absolute -left-4 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white border border-border-default shadow-sm flex items-center justify-center z-10 group-focus-within:bg-brand-primary group-focus-within:text-white transition-all">
                                            ...
                                            <div className="space-y-8">
                                                <div className="p-10 bg-brand-secondary rounded-[2.5rem] text-white shadow-mx-xl space-y-6">
                                                    <Typography variant="h3" tone="white" className="uppercase tracking-tight">Prazo de Revisão</Typography>
                                                    <Typography variant="tiny" tone="white" className="opacity-60 font-black">QUANDO ESTE CICLO DE EVOLUÇÃO SERÁ AUDITADO PELO GESTOR?</Typography>
                                                    <input 
                                                        type="date" 
                                                        value={form.due_date}
                                                ...
                                                <Card className="p-8 border-2 border-dashed border-border-default bg-surface-alt/50 flex flex-col items-center justify-center text-center space-y-4 rounded-mx-2xl">
                                                    ...
                                                    <Typography variant="tiny" tone="muted" className="font-black uppercase">O PDI deve ser impresso e assinado por ambas as partes para validade jurídica e tática.</Typography>
                                                </Card>
                                            </div>
                                        </div>
                                    </motion.div>
                                        ...
                                        <Card className="p-8 h-full flex flex-col justify-between group hover:shadow-mx-xl transition-all border-none shadow-mx-lg bg-white relative overflow-hidden">
                                            ...
                                            <div>
                                                <header className="flex items-start justify-between mb-10 border-b border-border-default pb-6 relative z-10">
                                                    ...
                                                    <Badge variant={status.variant} className="px-4 py-1 rounded-lg text-tiny font-black shadow-sm uppercase border-none">{status.label}</Badge>
                                                </header>

                                                <div className="space-y-8 relative z-10">
                                                    <div className="space-y-2">
                                                        <Typography variant="caption" tone="brand" className="font-black uppercase tracking-widest mb-2 block">Objetivo 06 Meses</Typography>
                                                        <Typography variant="h2" className="text-xl leading-snug line-clamp-2 uppercase tracking-tight">"{(p as any).meta_6m || (p as any).objective}"</Typography>
                                                    </div>
                                                    ...
                                                    <Card className="p-6 bg-surface-alt border-none shadow-inner group-hover:bg-white group-hover:shadow-mx-sm transition-all">
                                                        <header className="flex items-center justify-between mb-4 border-b border-border-strong/10 pb-3">
                                                            <Typography variant="tiny" tone="muted" className="font-black uppercase tracking-widest">Ação Prioritária #1</Typography>
