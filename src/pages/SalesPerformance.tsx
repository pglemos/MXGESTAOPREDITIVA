    ...
    return (
        <div className="w-full h-full flex flex-col gap-mx-lg overflow-y-auto no-scrollbar relative p-mx-md sm:p-mx-lg md:p-mx-xl text-text-primary">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-mx-lg shrink-0 pb-mx-3xl">
                ...
                <div className="lg:col-span-4"><Card className="h-full flex flex-col"><CardHeader><CardTitle>Mix por Categoria</CardTitle><CardDescription>Distribuição de Ativos</CardDescription></CardHeader><div className="p-mx-lg flex-1 flex flex-col items-center justify-center">
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={mockCategorySales} layout="vertical" margin={{ left: -20, right: 30 }}>
                                ...
                                <Bar dataKey="value" radius={[0, 8, 8, 0]} barSize={24}>
                                    {mockCategorySales.map((_, i) => (<Cell key={i} fill={i === 0 ? '#4f46e5' : i === 1 ? '#6366f1' : '#818cf8'} />))}
                                </Bar>
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                    <div className="w-full mt-mx-lg space-y-2">
                        {mockCategorySales.map((cat, i) => (
                            <div key={i} className="flex items-center justify-between p-3 rounded-mx-md bg-mx-slate-50 border border-border-subtle group hover:bg-white transition-all">
                                <div className="flex items-center gap-2"><div className={cn("w-2 h-2 rounded-full", i === 0 ? "bg-brand-primary" : "bg-status-info")} /><span className="text-[10px] font-black uppercase tracking-tight">{cat.category}</span></div>
                                <span className="text-xs font-black font-mono-numbers text-text-tertiary">{cat.value}%</span>
                            </div>
                        ))}
                    </div>
                </div></Card></div>
