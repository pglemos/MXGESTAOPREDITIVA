...
export default function Historico() {
    ...
    return (
        <main className="w-full h-full flex flex-col gap-10 overflow-y-auto no-scrollbar relative text-pure-black p-4 sm:p-6 md:p-10">
            ...
            {/* Header / 14. Typography standardized */}
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 relative z-10 w-full shrink-0 border-b border-gray-100 pb-10">
                <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-4">
                        <div className="w-2 h-10 bg-electric-blue rounded-full shadow-[0_0_20px_rgba(79,70,229,0.4)]" aria-hidden="true" />
                        <h1 className="text-4xl md:text-5xl font-black tracking-tighter leading-none">
                            Histórico <span className="text-electric-blue">Operacional</span>
                        </h1>
                    </div>
                    <div className="flex items-center gap-3 pl-6 mt-2">
                        ...
                        <p className="text-gray-500 text-xs font-black uppercase tracking-[0.4em]">
                            {processedCheckins.length} registros sincronizados
                        </p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4">
                    <div className="relative group w-full sm:w-64">
                        <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-electric-blue transition-colors" aria-hidden="true" />
                        <label htmlFor="history-search" className="sr-only">Buscar no histórico</label>
                        <input 
                            id="history-search"
                            type="text"
                        ...
                        />
                        {searchTerm && <button onClick={() => setSearchTerm('')} aria-label="Limpar busca" className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-300 hover:text-red-500 focus-visible:outline-none"><X size={14} /></button>}
                    </div>
                    
                    <div className="bg-gray-100/50 p-1 rounded-2xl flex border border-gray-100 shadow-inner">
                        <button 
                            onClick={() => setSortOrder(prev => prev === 'desc' ? 'asc' : 'desc')}
                            className="w-10 h-10 rounded-xl flex items-center justify-center bg-white text-pure-black shadow-sm hover:text-electric-blue transition-all focus-visible:ring-4 focus-visible:ring-indigo-500/10 outline-none"
            ...
            {checkins.length === 0 ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center py-40 rounded-[2.5rem] text-center border-dashed border-2 border-gray-200 bg-gray-50/30 relative overflow-hidden group">
                    ...
                    <h2 className="text-3xl font-black text-pure-black mb-4 tracking-tighter">Memória Vazia</h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-32" aria-live="polite">
                    <AnimatePresence mode="popLayout">
                        {processedCheckins.map((c, i) => (
                            ...
                            >
                                <div className="absolute top-0 right-0 w-48 h-48 bg-indigo-50/30 rounded-full blur-[60px] -mr-24 -mt-24 pointer-events-none group-hover:bg-indigo-50/50 transition-colors" aria-hidden="true" />
                                ...
                                <div className="mb-10 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between relative z-10">
                                    <div className="flex items-center gap-5">
                                        <div className="w-14 h-14 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-center shadow-inner group-hover:bg-pure-black transition-all group-hover:rotate-3" aria-hidden="true">
                                            <Calendar size={24} className="text-electric-blue group-hover:text-white transition-colors" />
                                        </div>
                                        <div className="min-w-0">
                                            <p className="text-xs font-black text-gray-500 uppercase tracking-[0.3em] mb-1 leading-none">Snapshot Operacional</p>
                                            <h2 className="text-xl font-black tracking-tighter text-pure-black leading-tight truncate">
                                    ...
                                    {/* 7. & 9. Sales Badge & Unit consistency */}
                                    {c.totals.vnd_total > 0 ? (
                                        ...
                                        <div className="bg-amber-50 border border-amber-100 px-5 py-2.5 rounded-2xl flex items-center gap-2 shrink-0 self-start sm:self-auto">
                                ...
                                <div className="mb-10 grid grid-cols-2 gap-4 sm:grid-cols-4 relative z-10">
                                    <div className="bg-gray-50/50 rounded-2xl p-5 border border-gray-100 hover:bg-white hover:shadow-lg transition-all text-center group/stat" title="Leads Captados">
                                        ...
                                        <p className="text-xs font-black text-gray-500 uppercase tracking-widest mt-2">Leads</p>
                                    </div>
                                    <div className="bg-gray-50/50 rounded-2xl p-5 border border-gray-100 hover:bg-white hover:shadow-lg transition-all text-center group/stat" title="Agendamentos">
                                        ...
                                        <p className="text-xs font-black text-gray-500 uppercase tracking-widest mt-2">Agd</p>
                                    </div>
                                    <div className="bg-gray-50/50 rounded-2xl p-5 border border-gray-100 hover:bg-white hover:shadow-lg transition-all text-center group/stat" title="Visitas Realizadas">
                                        ...
                                        <p className="text-xs font-black text-gray-500 uppercase tracking-widest mt-2">Visitas</p>
                                    </div>
                                    <div className="bg-gray-50/50 rounded-2xl p-5 border border-gray-100 hover:bg-white hover:shadow-lg transition-all text-center group/stat" title="Fechamentos">
                                        ...
                                        <p className="text-xs font-black text-gray-500 uppercase tracking-widest mt-2">Vendas</p>
                                    </div>
                                </div>
                                ...
                                <div className="mt-auto flex flex-col gap-6 border-t border-gray-100 pt-8 sm:flex-row sm:items-center sm:justify-between relative z-10">
                                    <div className="flex flex-wrap items-center gap-5 text-xs font-black uppercase tracking-widest text-gray-500">
                                    ...
                                    {c.note && (
                                        <div className="flex items-center gap-3 group/note relative cursor-help shrink-0">
                                            <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center text-gray-400 group-hover/note:text-electric-blue group-hover/note:bg-indigo-50 transition-all border border-gray-100" aria-hidden="true">
                                                <MessageSquare size={14} />
                                            </div>
                                            <span className="text-xs font-bold text-gray-500 italic truncate max-w-[140px]">{c.note}</span>

                                            {/* 3. & 4. Fixed Tooltip Position and Z-Index */}
                                            <div className="absolute bottom-full right-0 mb-4 w-64 p-5 bg-pure-black text-white text-xs font-bold rounded-[1.5rem] opacity-0 group-hover/note:opacity-100 transition-all pointer-events-none z-[60] shadow-3xl transform translate-y-2 group-hover/note:translate-y-0 border border-white/10 leading-relaxed italic">
                                                <span className="sr-only">Nota do registro: </span>
                                                "{c.note}"
                                                <div className="absolute top-full right-4 w-3 h-3 bg-pure-black rotate-45 -mt-1.5" aria-hidden="true" />
                                            </div>
                                        </div>
                                    )}
                                </div>
