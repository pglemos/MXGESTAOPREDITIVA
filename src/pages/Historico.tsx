        ...
        <main className="w-full h-full flex flex-col gap-mx-lg p-mx-lg overflow-y-auto no-scrollbar bg-surface-alt">
            ...
            <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-mx-lg border-b border-border-default pb-10 shrink-0">
                ...
                <div className="flex flex-wrap items-center gap-mx-sm shrink-0">
                    ...
                    <Button variant="outline" size="icon" onClick={() => setSortOrder(prev => prev === 'desc' ? 'asc' : 'desc')} className="rounded-xl h-12 w-12 shadow-mx-sm">
                        <ArrowUpDown size={18} />
                    </Button>
            ...
            {processedCheckins.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center py-40 rounded-[3rem] text-center border-dashed border-2 border-border-default bg-white/50 shadow-inner">
                    ...
                    <AnimatePresence mode="popLayout">
                        {processedCheckins.map((c, i) => (
                            <motion.div key={c.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.01 }}>
                                <Card className="p-8 md:p-10 group hover:shadow-mx-xl transition-all border-none shadow-mx-lg bg-white relative overflow-hidden">
                                    <div className="absolute top-0 right-0 w-32 h-32 bg-brand-primary/5 rounded-full blur-3xl -mr-16 -mt-16" />
                                    
                                    <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8 border-b border-border-default pb-8 relative z-10">
                                        <div className="flex items-center gap-4">
                                            <div className="w-14 h-14 rounded-mx-xl bg-surface-alt border border-border-default flex items-center justify-center text-brand-primary shadow-inner group-hover:bg-brand-primary group-hover:text-white transition-all">
                                                <Calendar size={24} />
                                            </div>
                                            <div>
                                        ...
                                        <Badge variant={c.totals.vnd_total > 0 ? 'success' : c.zero_reason ? 'warning' : 'outline'} className="px-6 py-2 rounded-full shadow-mx-sm font-black uppercase tracking-widest">
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10 relative z-10">
                                        {[
                                            ...
                                            <div key={stat.label} className="bg-surface-alt rounded-mx-2xl p-5 text-center border border-border-default shadow-inner group-hover:bg-white group-hover:shadow-mx-sm transition-all">
                                                ...
                                                <Typography variant="caption" tone="muted" className="text-[8px] uppercase tracking-widest">{stat.label}</Typography>
                                            </div>
                                        ))}
                                    </div>

                                    {c.note && (
                                        <footer className="pt-8 border-t border-border-default flex items-start gap-4 relative z-10">
                                            <div className="w-8 h-8 rounded-full bg-mx-indigo-50 flex items-center justify-center text-brand-primary shrink-0 shadow-inner">
