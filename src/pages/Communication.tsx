    ...
    <main className="w-full h-full flex flex-col gap-mx-lg p-4 md:p-mx-lg overflow-y-auto no-scrollbar bg-surface-alt">
      ...
      <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-mx-lg border-b border-border-default pb-10 shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-mx-sm shrink-0">
          <Button variant="outline" size="icon" onClick={() => {setIsRefetching(true); setTimeout(() => {setIsRefetching(false); toast.success('Sincronizado!')}, 800)}} className="w-mx-xl h-mx-xl rounded-mx-xl shadow-mx-sm bg-white">
          ...
          <Button className="h-mx-xl px-8 shadow-mx-lg bg-brand-secondary uppercase font-black tracking-widest">
          ...
          <Card className="border-none shadow-mx-xl bg-white overflow-hidden h-full flex flex-col group relative">
            ...
            <CardHeader className="bg-surface-alt/30 border-b border-border-default p-mx-10 flex flex-row items-center justify-between relative z-10">
              <div className="flex items-center gap-mx-md">
                <div className="w-mx-2xl h-mx-2xl rounded-mx-2xl bg-mx-black text-white flex items-center justify-center shadow-mx-xl"><Megaphone size={32} /></div>
                <div>
              ...
              <Badge variant="success" className="px-6 py-2 rounded-mx-full font-black shadow-mx-sm uppercase">Rede Ativa</Badge>
            </CardHeader>
                ...
                <div className="space-y-mx-14">
                  <AnimatePresence mode="popLayout">
                    {filteredMessages.map((m, i) => (
                      <motion.div key={m.id} layout initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: i * 0.03 }} className="relative z-10 flex gap-mx-10 group/msg">
                        ...
                        <div className="flex-1 bg-surface-alt/50 border border-border-default rounded-mx-3xl p-4 md:p-mx-lg hover:bg-white hover:shadow-mx-xl transition-all relative">
                          <header className="flex justify-between items-start mb-6">
                            <div className="space-y-mx-tiny">
                              <div className="flex items-center gap-mx-xs">
                                ...
                                <Badge variant="outline" className="rounded-mx-md border-border-strong opacity-60 uppercase font-black">{m.role}</Badge>
                              </div>
                              <Typography variant="mono" tone="muted" className="opacity-40 uppercase font-black tracking-widest">{m.date}</Typography>
                            </div>
                            <Badge variant={m.priority === 'High' ? 'danger' : 'outline'} className="px-4 py-1 rounded-mx-lg uppercase font-black shadow-sm">{m.priority}</Badge>
                          </header>
                          ...
                          <footer className="flex items-center gap-mx-md mt-8">
