    ...
    return (
        <main className="w-full h-full flex flex-col gap-mx-lg p-mx-lg overflow-y-auto no-scrollbar bg-surface-alt">
            ...
            <AnimatePresence>
                {activeTab === 'seller' && (
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="shrink-0 mb-4">
                        <div className="flex items-center gap-4 bg-white p-4 rounded-3xl border border-border-default shadow-mx-sm">
                            <div className="w-10 h-10 rounded-full bg-mx-indigo-50 flex items-center justify-center text-brand-primary"><User size={18} /></div>
                            <div className="flex-1 relative group">
                                <select 
                                    value={selectedSellerId}
                                    onChange={(e) => setSelectedSellerId(e.target.value)}
                                    className="w-full h-12 px-6 bg-surface-alt border border-border-default rounded-2xl text-xs font-black uppercase focus:border-brand-primary transition-all appearance-none cursor-pointer"
                                >
                                    <option value="">Selecione um especialista...</option>
                                    {sellers.map(s => <option key={s.id} value={s.id}>{s.name.toUpperCase()}</option>)}
                                </select>
                                <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-text-tertiary pointer-events-none group-hover:text-brand-primary transition-colors" />
                            </div>
                        </div>
                    </motion.div>
                )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-mx-lg shrink-0">
                <Card className="lg:col-span-8 bg-brand-secondary text-white p-10 md:p-14 shadow-mx-xl relative overflow-hidden border-none group">
                    <div className="absolute top-0 right-0 w-[400px] h-full bg-gradient-to-l from-white/5 via-transparent to-transparent pointer-events-none" aria-hidden="true" />
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
                        <div className="w-20 h-20 rounded-mx-3xl bg-white text-brand-primary flex items-center justify-center shadow-mx-xl group-hover:rotate-12 transition-transform shrink-0" aria-hidden="true">
                ...
                <Card className="lg:col-span-4 p-10 md:p-14 flex flex-col justify-between border-none shadow-mx-lg bg-white relative overflow-hidden group">
                    ...
                    <div className="space-y-2 relative z-10">
                        <Typography variant="caption" tone="muted" className="uppercase font-black tracking-[0.2em]">Eficiência Global</Typography>
                    ...
                    <div className="pt-10 border-t border-border-default mt-10 relative z-10">
                        ...
                        <div className="h-2.5 w-full bg-surface-alt rounded-mx-full overflow-hidden border border-border-default shadow-inner p-0.5">
                            <motion.div 
                                ...
                                className={cn("h-full rounded-full shadow-mx-sm transition-all duration-1000", funilData.tx_visita_vnd >= 33 ? "bg-status-success shadow-[0_0_15px_rgba(16,185,129,0.5)]" : "bg-status-error")} 
                            />
                        </div>
                    </div>
                </Card>
                ...
                {[
                    ...
                    <Card key={item.label} className="p-8 border-none shadow-mx-sm group hover:shadow-mx-lg transition-all bg-white relative overflow-hidden">
                        <div className="flex items-center justify-between mb-8 relative z-10">
                            <div className={cn("w-14 h-14 rounded-mx-2xl flex items-center justify-center border shadow-inner group-hover:scale-110 transition-transform", 
            ...
            <Card className="w-full mb-32 border-none shadow-mx-lg bg-white overflow-hidden group">
                <CardHeader className="bg-surface-alt/30 border-b border-border-default p-10">
                ...
                <CardContent className="p-10 md:p-14">
                    <div className="flex flex-col gap-14">
                        {[
                            ...
                            <div key={idx} className="flex flex-col md:flex-row items-center gap-14 relative">
                                <div className="flex-1 w-full space-y-6">
                                    <div className="flex justify-between items-end">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-mx-lg bg-surface-alt flex items-center justify-center font-black text-text-tertiary text-xs border border-border-default shadow-sm group-hover:bg-brand-primary group-hover:text-white transition-all">0{idx+1}</div>
                                    ...
                                    <div className="h-4 w-full bg-surface-alt rounded-mx-full overflow-hidden border border-border-default shadow-inner p-1">
                                        <motion.div 
                                            initial={{ width: 0 }} animate={{ width: `${Math.min(step.val, 100)}%` }} transition={{ duration: 1.5, delay: idx * 0.2, ease: "circOut" }}
                                            className={cn("h-full rounded-full shadow-mx-sm transition-all duration-1000", 
                                                step.val >= step.bench ? 
                                ...
                                {idx < 2 && <div className="hidden md:flex flex-col items-center gap-2 opacity-20"><ArrowRight size={40} strokeWidth={3} /></div>}
                            </div>
                        ))}
                    </div>
                </CardContent>
