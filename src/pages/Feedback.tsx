...
import { MessageSquare, Star, TrendingUp, Target, User, MessageCircle, Award, RefreshCw } from 'lucide-react'
...
export default function Feedback() {
  ...
  const stats = [
    ...
    { label: 'Total Feedbacks', value: feedbacks.length.toString(), icon: MessageCircle, tone: 'bg-brand-primary-surface text-brand-primary' },
    ...
    <div className="w-full h-full flex flex-col gap-mx-lg overflow-y-auto no-scrollbar relative p-mx-md sm:p-mx-lg md:p-mx-xl text-text-primary">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-mx-lg border-b border-border-default pb-mx-lg shrink-0">
        ...
        <div className="flex items-center gap-mx-sm shrink-0">
          <button onClick={handleRefetch} className="w-12 h-12 rounded-mx-lg bg-white border border-border-default shadow-mx-sm flex items-center justify-center text-text-tertiary hover:text-text-primary"><RefreshCw size={20} className={cn(isRefetching && "animate-spin")} /></button>
      ...
      {loading ? (
        <div className="flex-1 flex items-center justify-center">
            <p className="text-gray-400 text-[10px] font-black tracking-[0.4em] uppercase">Carregando Feedbacks...</p>
        </div>
      ) : (
        <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-mx-lg flex-1 min-h-0 pb-mx-3xl">
                ...
                <div className="mx-card h-full flex flex-col overflow-hidden group">
                    ...
                    <AnimatePresence mode="popLayout">
                        ...
                        <motion.div key={f.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="p-mx-lg rounded-mx-2xl border bg-white border-mx-slate-100 transition-all hover:shadow-mx-lg group/item relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-mx-md">
                                ...
                                <div><h4 className="font-black text-sm text-text-primary uppercase tracking-tight">{f.manager_name || 'Gestão'}</h4><p className="mx-text-caption !text-[8px] opacity-60">{new Date(f.created_at).toLocaleDateString()} • Feedback</p></div>
                            </div>
                            <div className="flex items-center gap-1">
                                {[1, 2, 3, 4, 5].map(s => <Star key={s} size={12} className={cn(s <= (f.score || 5) ? "text-status-warning fill-current" : "text-mx-slate-200")} />)}
                            </div>
                            </div>
