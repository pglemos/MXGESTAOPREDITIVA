  ...
  return (
    <main className="w-full h-full flex flex-col gap-mx-lg p-mx-lg overflow-y-auto no-scrollbar bg-surface-alt">
      ...
      <Card className="flex-1 overflow-hidden flex flex-col border-none shadow-mx-xl bg-white mb-20">
        <CardHeader className="p-10 border-b border-border-default bg-surface-alt/30 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-3 bg-white px-6 py-2.5 rounded-full border border-border-default shadow-mx-sm">
                ...
                {filteredLeads.map((lead) => (
                  <React.Fragment key={lead.id}>
                    <tr 
                        ...
                        className={cn('hover:bg-surface-alt/30 transition-colors cursor-pointer group h-24', expandedLead === lead.id && 'bg-mx-indigo-50/30')}
                    >
                      <td className="pl-10">
                        <div className="flex items-center gap-4">
                          <div className="h-12 w-12 rounded-mx-xl bg-brand-secondary text-white flex items-center justify-center font-black text-sm shadow-mx-lg group-hover:rotate-6 transition-transform">{lead.name.charAt(0)}</div>
                          <div className="min-w-0">
                            ...
                            <Typography variant="caption" tone="muted" className="text-[10px] font-black">{lead.phone}</Typography>
                          </div>
                        </div>
                      </td>
                      <td className="px-6">
                        <Typography variant="h3" className="text-sm uppercase tracking-tight leading-none mb-1">{lead.vehicle}</Typography>
                        <Typography variant="caption" tone="muted" className="text-[8px] font-black uppercase opacity-60">{lead.source}</Typography>
                      </td>
                      <td className="px-6">
                        <div className="flex items-center justify-center gap-2">
                          <div className={cn('flex items-center justify-center h-10 w-10 rounded-mx-xl font-black text-sm border shadow-inner', 
                      ...
                      <td className="px-6">
                        <div className="flex flex-col items-center gap-2">
                          <Badge variant="outline" className="text-[8px] font-black px-3 h-5 border-border-strong">{lead.status.toUpperCase()}</Badge>
                      ...
                      <td className="px-6 text-center">
                        <Typography variant="caption" tone="muted" className="text-[9px] font-black opacity-70 uppercase">{lead.owner}</Typography>
                      </td>
                      <td className="pr-10 text-right">
                        <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                          <Button variant="ghost" size="icon" onClick={() => window.open(`tel:${lead.phone}`)} className="w-10 h-10 rounded-xl text-text-tertiary hover:text-brand-primary hover:bg-mx-indigo-50"><Phone size={18} /></Button>
                          <Button variant="ghost" size="icon" onClick={() => handleWhatsApp(lead.phone)} className="w-10 h-10 rounded-xl text-text-tertiary hover:text-status-success hover:bg-status-success-surface"><MessageSquare size={18} /></Button>
                          <Button variant="ghost" size="icon" className="w-10 h-10 rounded-xl text-text-tertiary"><MoreVertical size={18} /></Button>
                        </div>
                      </td>
                    </tr>
                    <AnimatePresence>
                        ...
                        <motion.tr initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-surface-alt/30">
                            <td colSpan={6} className="p-0 overflow-hidden">
                                <div className="p-10 md:p-14 border-t border-border-default flex flex-col md:flex-row items-start gap-10">
                                    <div className="w-16 h-16 rounded-mx-2xl bg-brand-secondary text-white flex items-center justify-center shadow-mx-xl transform -rotate-3 shrink-0"><Sparkles size={32} /></div>
                                    <div className="flex-1 space-y-6">
                                        ...
                                        <div className="flex gap-4 pt-4">
                                            <Button className="h-12 px-8 rounded-full shadow-mx-lg">EXECUTAR PLANO</Button>
                                            <Button variant="outline" className="h-12 px-8 rounded-full shadow-sm bg-white">VER LOGS</Button>
                                        </div>
                                    </div>
                                </div>
                            </td>
