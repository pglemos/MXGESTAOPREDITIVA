    ...
    <main className="w-full h-full flex flex-col gap-mx-lg p-4 md:p-mx-lg overflow-y-auto no-scrollbar bg-surface-alt">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-mx-lg flex-1 min-h-0 pb-32">
        ...
        <section className="lg:col-span-4 flex flex-col gap-mx-lg">
            ...
            <div className="space-y-mx-10">
              <div className="space-y-mx-sm">
                ...
                <div className="relative group">
                  <select
                    className="w-full h-mx-14 bg-surface-alt border border-border-default rounded-mx-xl px-6 text-sm font-bold text-text-primary outline-none focus:border-brand-primary focus:ring-4 focus:ring-brand-primary/5 transition-all appearance-none cursor-pointer shadow-inner"
                  ...
                  <Smartphone size={18} className="absolute right-mx-sm top-1/2 -translate-y-1/2 text-text-tertiary pointer-events-none group-hover:text-brand-primary transition-colors" />
                </div>
              </div>
          ...
          {/* Discipline Card */}
          <Card className="p-mx-10 border-none shadow-mx-lg bg-white space-y-mx-10 relative overflow-hidden group">
            ...
            <div className="space-y-mx-10 relative z-10">
              <div className="space-y-mx-sm">
                ...
                <div className="w-full bg-surface-alt border border-border-default rounded-mx-full h-mx-xs overflow-hidden p-0.5 shadow-inner">
                  <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${executionRate}%` }}
                    transition={{ duration: 1.5, ease: "circOut" }}
                    className="bg-brand-primary h-full rounded-mx-full shadow-mx-sm" 
                  />
                </div>
              </div>
          ...
          <Card className="bg-white border-none shadow-mx-xl overflow-hidden h-full flex flex-col group relative">
            ...
            <CardContent className="flex-1 overflow-y-auto no-scrollbar p-mx-10 md:p-14 relative z-10">
              <div className="relative">
                <div className="absolute left-mx-7 top-mx-0 bottom-mx-0 w-px bg-border-default/50 z-0" />
                
                <div className="space-y-mx-14">
                  <AnimatePresence mode="popLayout">
                    {filteredActivities.map((activity, idx) => (
                      <motion.div
                        key={activity.id}
                        layout initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: idx * 0.03 }}
                        className="relative z-10 flex gap-mx-10 group/item"
                      >
                        <div className={cn(
                          "w-mx-14 h-mx-14 rounded-mx-xl shrink-0 flex items-center justify-center border-4 border-white shadow-mx-lg transition-transform group-hover/item:scale-110 group-hover/item:rotate-3",
                          ...
                          <header className="flex justify-between items-start mb-6">
                            <div>
                              ...
                              <div className="flex items-center gap-mx-xs">
                                ...
                                <Typography variant="caption" tone="muted" className="text-mx-micro font-black uppercase tracking-widest">Showroom Alpha</Typography>
                              </div>
                            </div>
                            <Badge variant="outline" className="font-mono-numbers px-4 py-1.5 rounded-mx-lg bg-white shadow-mx-sm border-border-default">{activity.time}</Badge>
                          </header>
                          
                          <footer className="flex items-center justify-between gap-mx-md border-t border-border-default pt-6">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => deleteActivity(activity.id)} className="opacity-0 group-hover/item:opacity-100 text-text-tertiary hover:text-status-error transition-all p-mx-xs h-mx-10 w-mx-10 rounded-mx-full">
                              <Trash2 size={18} />
                            </Button>
                          </footer>
                        </div>
