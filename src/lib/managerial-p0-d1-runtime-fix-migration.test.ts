...
describe('managerial P0 D+1 runtime fix', () => {
  ...
  test('requires an explicit audited confirmation mutation', () => {
    ...
    expect(compactSql).toContain("p_confirmation_status NOT IN ('pendente','whatsapp_aberto','confirmado','sem_resposta','solicitou_reagendamento','cancelou','outro')")
  ...
  test('audits WhatsApp and phone actions without pretending the message was sent', () => {
    ...
    expect(compactSql).toContain("action_type text NOT NULL CHECK (action_type IN ( 'whatsapp_opened','phone_opened','confirmation_changed','seller_notified' ))")
