...
SELECT is((WITH i AS (INSERT INTO public.lojas (id,name,active) VALUES (gen_random_uuid(),'x',true) RETURNING 1) SELECT count(*) FROM i), 0::bigint, 'dono: INSERT bloqueado');
SELECT is((WITH u AS (UPDATE public.lojas SET name=name RETURNING 1) SELECT count(*) FROM u), 0::bigint, 'dono: UPDATE bloqueado');
SELECT is((WITH d AS (DELETE FROM public.lojas RETURNING 1) SELECT count(*) FROM d), 0::bigint, 'dono: DELETE bloqueado');
...
SELECT is((WITH i AS (INSERT INTO public.lojas (id,name,active) VALUES (gen_random_uuid(),'x',true) RETURNING 1) SELECT count(*) FROM i), 0::bigint, 'gerente: INSERT bloqueado');
SELECT is((WITH u AS (UPDATE public.lojas SET name='x' WHERE id='11111111-1111-1111-1111-111111111111' RETURNING 1) SELECT count(*) FROM u), 0::bigint, 'gerente: UPDATE bloqueado');
SELECT is((WITH d AS (DELETE FROM public.lojas RETURNING 1) SELECT count(*) FROM d), 0::bigint, 'gerente: DELETE bloqueado');
...
SELECT is((WITH i AS (INSERT INTO public.lojas (id,name,active) VALUES (gen_random_uuid(),'x',true) RETURNING 1) SELECT count(*) FROM i), 0::bigint, 'vendedor: INSERT bloqueado');
SELECT is((WITH u AS (UPDATE public.lojas SET name='hack' RETURNING 1) SELECT count(*) FROM u), 0::bigint, 'vendedor: UPDATE bloqueado');
SELECT is((WITH d AS (DELETE FROM public.lojas RETURNING 1) SELECT count(*) FROM d), 0::bigint, 'vendedor: DELETE bloqueado');
...
SELECT is((WITH u AS (UPDATE public.lojas SET name='hack' RETURNING 1) SELECT count(*) FROM u), 0::bigint, 'anon: UPDATE bloqueado');
