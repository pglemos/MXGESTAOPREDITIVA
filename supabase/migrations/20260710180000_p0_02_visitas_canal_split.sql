-- ============================================================================
-- Migration: 20260710180000_p0_02_visitas_canal_split.sql
-- Origem:    Auditoria Integral 2026-07-10 (P0-02)
--
-- ESCOPO: `lancamentos_diarios.visit_prev_day` só guarda o total de visitas;
--   a distribuição por canal (Showroom/Porta, Carteira, Internet) digitada
--   pelo vendedor no Fechamento Diário nunca era persistida — o client
--   (useCheckinsSubmit) só enviava o total somado. Ao reabrir um fechamento
--   salvo (Histórico/Regularização), o hook então jogava esse total inteiro
--   em "visitas_porta" e zerava "visitas_cart"/"visitas_net" — perdendo a
--   distribuição real do vendedor.
--
--   Aditivo: 3 colunas novas (nullable — NULL = "distribuição não rastreada",
--   nunca deve virar zero em registros anteriores a esta migração). Sem
--   backfill: não existe fonte auditável para reconstruir o canal de visitas
--   de registros antigos, e "inventar" a distribuição violaria o Artigo IV
--   (No Invention) da Constitution do projeto.
--
--   `visit_prev_day` continua sendo o total (compatibilidade com ranking,
--   performance, funil, etc. — nenhum desses consumidores muda). Quando o
--   client envia os 3 canais, o servidor recalcula o total a partir deles em
--   vez de confiar no total enviado (defesa em profundidade, mesmo padrão já
--   usado para a disciplina/EV-1.5).
--
...
ALTER TABLE public.lancamentos_diarios
  ADD COLUMN IF NOT EXISTS visitas_porta_prev_day integer,
  ADD COLUMN IF NOT EXISTS visitas_cart_prev_day  integer,
  ADD COLUMN IF NOT EXISTS visitas_net_prev_day   integer;

COMMENT ON COLUMN public.lancamentos_diarios.visitas_porta_prev_day IS
  'Visitas via Showroom/Porta (P0-02). NULL = distribuição por canal não rastreada (registro anterior a 2026-07-10) — não confundir com zero.';
COMMENT ON COLUMN public.lancamentos_diarios.visitas_cart_prev_day IS
  'Visitas via Carteira (P0-02). NULL = distribuição por canal não rastreada.';
COMMENT ON COLUMN public.lancamentos_diarios.visitas_net_prev_day IS
  'Visitas via Internet (P0-02). NULL = distribuição por canal não rastreada.';
...
-- BEGIN;
--   ALTER TABLE public.lancamentos_diarios
--     DROP COLUMN IF EXISTS visitas_porta_prev_day,
--     DROP COLUMN IF EXISTS visitas_cart_prev_day,
--     DROP COLUMN IF EXISTS visitas_net_prev_day;
--   -- Reverter submit_checkin para a versão de
