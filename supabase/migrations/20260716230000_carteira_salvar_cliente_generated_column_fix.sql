-- Carteira Base44: corrige carteira_salvar_cliente para não escrever em
-- telefone_normalizado (coluna GENERATED ALWAYS AS
-- (NULLIF(regexp_replace(COALESCE(telefone,''),'\D','','g'),'')) STORED).
--
-- A definição ao vivo desta função (confirmada via pg_get_functiondef) tenta
-- INSERT/UPDATE explícito em telefone_normalizado nas listas de colunas, o
-- que o Postgres rejeita com 428C9 "cannot insert a non-DEFAULT value into
-- column" para qualquer coluna gerada. Nenhuma migration commitada no
-- histórico contém essa versão — nem 20260716050241 nem 20260716190050
-- (esta última já não escreve na coluna, só lê via COALESCE) — ou seja, a
-- função ao vivo divergiu do histórico versionado em algum momento fora de
-- uma migration rastreada. Reproduzido em produção: toda chamada de
-- carteira_salvar_cliente_v2 (adapter -> saveClient, usado por
-- FichaClienteSheet, NovoClienteModal, AlterarProximoPasso e WhatsAppRoteiro)
-- falhava com HTTP 400 antes desta correção.
--
-- Corpo idêntico ao vivo, exceto: remove telefone_normalizado da lista de
-- colunas/valores do INSERT e remove a atribuição telefone_normalizado do
-- UPDATE SET — a coluna gerada já é recalculada automaticamente a partir de
-- telefone em ambos os casos.
...
-- DOWN: use supabase/rollbacks/20260716230000_carteira_salvar_cliente_generated_column_fix.sql.
