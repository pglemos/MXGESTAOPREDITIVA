-- Corrige divergência de fonte de dados na Meta da Loja.
--
-- consolidate_store_target_plan calculava "realized" somando lancamentos_diarios
-- (fechamento diário do vendedor), enquanto a tabela de Contribuição da Equipe e o
-- fallback do dashboard usam vendedor_performance_oficial, que soma eventos_comerciais
-- (venda_realizada, originada de oportunidades ganhas no CRM). As duas fontes divergem
-- sempre que a venda existe no CRM mas o vendedor ainda não fez o fechamento diário do
-- período — a Meta da Loja exibia 0/realizado enquanto a tabela de vendedores já
-- mostrava vendas. Alinha ao read model único (eventos_comerciais) descrito na
-- migration 20260710150000_official_seller_performance.sql.
...
BEGIN
  ...
  -- Fonte alinhada com vendedor_performance_oficial: vendas realizadas (CRM),
  -- não fechamento diário. Evita divergência entre a Meta da Loja e a
  -- Contribuição da Equipe quando o checkin do dia ainda não foi enviado.
  SELECT COUNT(*)::numeric
  INTO realized_value
  FROM public.eventos_comerciais ec
  WHERE ec.loja_id = p_store_id
    AND ec.tipo_evento = 'venda_realizada'
    AND COALESCE(ec.data_competencia, timezone('America/Sao_Paulo', ec.data_evento)::date)
      BETWEEN month_start AND target_reference_date;
