...
ALTER TABLE public.execution_actions
  ADD CONSTRAINT execution_actions_source_type_check CHECK (
    source_type = ANY (ARRAY[
      'pdi', 'feedback', 'funil', 'manual', 'agendamento', 'cliente', 'sistema'
    ]::text[])
  ) NOT VALID,
  ADD CONSTRAINT execution_actions_status_check CHECK (
    status = ANY (ARRAY[
      'pendente', 'em_andamento', 'concluida', 'justificada', 'reagendada', 'cancelada'
    ]::text[])
  ) NOT VALID,
  ...
  ADD CONSTRAINT execution_actions_priority_rank_check CHECK (
    priority_rank BETWEEN 1 AND 9
  ) NOT VALID,
...
ALTER TABLE public.execution_actions
  VALIDATE CONSTRAINT execution_actions_source_type_check;
ALTER TABLE public.execution_actions
  VALIDATE CONSTRAINT execution_actions_status_check;
ALTER TABLE public.execution_actions
  VALIDATE CONSTRAINT execution_actions_activity_type_check;
ALTER TABLE public.execution_actions
  VALIDATE CONSTRAINT execution_actions_priority_rank_check;
ALTER TABLE public.execution_actions
  VALIDATE CONSTRAINT execution_actions_result_code_check;
...
COMMENT ON COLUMN public.execution_actions.activity_type IS
  'Tipo visual e funcional usado pelos catalogos de resolucao da Central.';

CREATE OR REPLACE FUNCTION public.central_can_manage_action(
  p_seller_id uuid,
  p_store_id uuid
)
RETURNS boolean
...
CREATE OR REPLACE FUNCTION public.central_result_allowed(
...
AS $$
  SELECT CASE p_activity_type
    WHEN 'retorno' THEN p_result_code = ANY (ARRAY[
      'contacted', 'no_answer', 'no_response', 'reschedule', 'advanced', 'manager_required'
    ]::text[])
    WHEN 'entrega' THEN p_result_code = ANY (ARRAY[
      'delivery_completed', 'delivery_confirmed', 'reschedule', 'documentation_pending', 'client_absent'
    ]::text[])
    WHEN 'garantia' THEN p_result_code = ANY (ARRAY[
      'contacted', 'waiting_workshop', 'waiting_part', 'warranty_resolved', 'manager_required', 'reschedule'
    ]::text[])
    WHEN 'pos_venda' THEN p_result_code = ANY (ARRAY[
      'post_sale_satisfied', 'post_sale_question', 'complaint', 'repurchase', 'referral', 'reschedule'
    ]::text[])
    WHEN 'documentacao' THEN p_result_code = ANY (ARRAY[
      'documentation_completed', 'documentation_pending', 'reschedule', 'manager_required'
    ]::text[])
    WHEN 'comercial' THEN p_result_code = ANY (ARRAY[
      'task_completed', 'task_justified', 'reschedule', 'manager_required', 'cancelled'
    ]::text[])
    WHEN 'pdi' THEN p_result_code = ANY (ARRAY[
      'task_completed', 'task_justified', 'reschedule', 'cancelled'
    ]::text[])
    WHEN 'feedback' THEN p_result_code = ANY (ARRAY[
      'task_completed', 'task_justified', 'manager_required', 'cancelled'
    ]::text[])
    WHEN 'aniversario' THEN p_result_code = ANY (ARRAY[
      'contacted', 'no_answer', 'no_response', 'reschedule'
    ]::text[])
...
BEGIN
  ...
  v_status := CASE
    WHEN v_ag.status::text IN ('compareceu', 'nao_compareceu') THEN 'concluida'
    ELSE 'pendente'
  END;
  ...
  v_priority_rank := CASE v_ag.tipo::text
    ...
    WHEN 'entrega' THEN 2
    WHEN 'garantia' THEN 3
    WHEN 'retorno' THEN 4
    WHEN 'pos_venda' THEN 5
    ELSE 5
  END;
  v_priority := CASE
    WHEN v_priority_rank <= 1 THEN 'urgent'
    WHEN v_priority_rank <= 3 THEN 'high'
    ELSE 'medium'
  END;
  ...
  INSERT INTO public.execution_actions (
    ...
    v_ag.id,
    v_ag.cliente_id,
    v_ag.oportunidade_id,
    v_ag.id,
    v_ag.tipo::text,
    concat(
      CASE v_ag.tipo::text
        WHEN 'visita' THEN 'Atendimento'
        WHEN 'test_drive' THEN 'Test-drive'
        WHEN 'pos_venda' THEN 'Pos-venda'
        ELSE initcap(replace(v_ag.tipo::text, '_', ' '))
      END,
      ' - ',
    ...
    v_priority,
    v_priority_rank,
    CASE WHEN v_priority_rank <= 2 THEN 'warning' ELSE 'info' END,
    coalesce(nullif(v_ag.origem_modulo, ''), 'crm'),
    v_ag.id::text,
    v_key,
    v_active,
    true,
    v_ag.cliente_nome,
    v_ag.cliente_telefone,
    v_ag.veiculo_interesse,
    jsonb_build_object(
      'appointment_status', v_ag.status::text,
      'appointment_type', v_ag.tipo::text,
      'channel', v_ag.canal::text
    ),
    coalesce(v_ag.created_by, v_ag.seller_user_id),
    coalesce(v_ag.updated_by, v_ag.seller_user_id),
  ...
  DO UPDATE SET
    ...
    status = CASE
      WHEN public.execution_actions.status IN ('cancelada', 'justificada') THEN public.execution_actions.status
    ...
    active = CASE
      WHEN public.execution_actions.status IN ('cancelada', 'justificada') THEN false
...
BEGIN
  ...
  IF v_activity_type IS NULL OR NOT (
    v_activity_type = ANY (ARRAY[
      'atendimento', 'visita', 'retorno', 'documentacao', 'entrega', 'pos_venda',
      'aniversario', 'garantia', 'comercial', 'test_drive', 'negociacao',
      'pdi', 'feedback', 'funil'
    ]::text[])
  ...
  IF v_due_at IS NULL THEN
    RAISE EXCEPTION 'Data e horario obrigatorios.';
  END IF;
  ...
  IF v_store_id IS NOT NULL AND NOT public.central_can_manage_action(auth.uid(), v_store_id) THEN
    RAISE EXCEPTION 'Sem permissao para criar atividade nesta loja.';
  END IF;

  IF v_client_id IS NOT NULL THEN
    SELECT id, nome, telefone, loja_id, seller_user_id
    INTO v_client
    ...
    IF NOT public.central_can_manage_action(v_client.seller_user_id, v_client.loja_id) THEN
      RAISE EXCEPTION 'Sem permissao para usar este cliente.';
    END IF;

    v_store_id := coalesce(v_store_id, v_client.loja_id);
  END IF;
  ...
  INSERT INTO public.execution_actions (
    ...
    nullif(trim(coalesce(p_payload->>'objective', '')), ''),
    v_due_at,
    'pendente',
    v_priority,
    v_priority_rank,
    ...
    'central_execucao',
    nullif(trim(coalesce(p_payload->>'source_record_id', '')), ''),
    p_idempotency_key,
    true,
    false,
    coalesce(v_client.nome, nullif(trim(coalesce(p_payload->>'name_snapshot', '')), '')),
    coalesce(v_client.telefone, nullif(trim(coalesce(p_payload->>'phone_snapshot', '')), '')),
...
BEGIN
  ...
  IF p_due_at IS NULL THEN
    RAISE EXCEPTION 'Nova data e horario obrigatorios.';
  END IF;
  IF nullif(trim(coalesce(p_idempotency_key, '')), '') IS NULL THEN
    RAISE EXCEPTION 'Chave de idempotencia obrigatoria.';
  END IF;
  ...
  IF v_action.cliente_id IS NOT NULL THEN
    ...
    SET
      proxima_acao = coalesce(nullif(trim(coalesce(p_note, '')), ''), 'Retomar contato'),
...
BEGIN
  ...
  IF nullif(trim(coalesce(p_reason, '')), '') IS NULL THEN
    RAISE EXCEPTION 'Motivo do escalonamento obrigatorio.';
  END IF;
  IF nullif(trim(coalesce(p_idempotency_key, '')), '') IS NULL THEN
    RAISE EXCEPTION 'Chave de idempotencia obrigatoria.';
  END IF;
  ...
  WHERE vl.store_id = v_action.store_id
    AND vl.role = 'gerente'
    AND vl.is_active
    AND vl.ended_at IS NULL
  ORDER BY vl.created_at DESC
  ...
  IF v_manager_id IS NULL AND v_action.store_id IS NOT NULL THEN
    ...
    FROM public.lojas l
    JOIN public.usuarios u ON lower(u.email) = lower(l.manager_email)
    WHERE l.id = v_action.store_id
      AND u.active
    LIMIT 1;
  END IF;
  ...
  IF v_manager_id IS NOT NULL THEN
    INSERT INTO public.notificacoes (
      ...
      auth.uid(),
      'Vendedor precisa de apoio',
      concat(v_action.title, ': ', trim(p_reason)),
      'store',
      v_action.store_id,
      'gerente',
      v_manager_id,
...
BEGIN
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION 'Sessao invalida.';
  END IF;
  IF nullif(trim(coalesce(p_result_code, '')), '') IS NULL THEN
    RAISE EXCEPTION 'Resultado obrigatorio.';
  END IF;
  IF nullif(trim(coalesce(p_idempotency_key, '')), '') IS NULL THEN
    RAISE EXCEPTION 'Chave de idempotencia obrigatoria.';
  END IF;
  ...
  IF p_result_code = 'reschedule' THEN
    v_due_at := nullif(coalesce(p_payload, '{}'::jsonb)->>'due_at', '')::timestamptz;
  ...
  IF v_action.agendamento_id IS NOT NULL THEN
    ...
    SET
      ...
      END,
      observacoes = CASE
        WHEN nullif(trim(coalesce(p_note, '')), '') IS NULL THEN observacoes
        ELSE concat_ws(E'\n', observacoes, concat('Central: ', trim(p_note)))
      END,
      updated_by = auth.uid(),
      updated_at = now()
    WHERE id = v_action.agendamento_id;
  ...
  IF v_action.oportunidade_id IS NOT NULL AND p_result_code = 'sale_completed' THEN
    ...
    SET
      valor_negociado = coalesce(nullif(p_payload->>'value', '')::numeric, valor_negociado),
      ...
      etapa = 'ganho'::public.crm_etapa_funil,
      closed_at = now(),
      updated_by = auth.uid(),
      updated_at = now()
  ...
  ELSIF v_action.oportunidade_id IS NOT NULL AND p_result_code = 'sale_lost' THEN
    ...
    UPDATE public.oportunidades
    SET
      etapa = 'perdido'::public.crm_etapa_funil,
  ...
  ELSIF v_action.oportunidade_id IS NOT NULL AND p_result_code = 'advanced' THEN
    UPDATE public.oportunidades
    SET
      etapa = CASE WHEN etapa IN ('ganho', 'perdido') THEN etapa ELSE 'negociacao'::public.crm_etapa_funil END,
      updated_by = auth.uid(),
      updated_at = now()
    WHERE id = v_action.oportunidade_id;
  END IF;
  ...
  IF v_event_type IS NOT NULL AND v_action.cliente_id IS NOT NULL AND v_action.store_id IS NOT NULL THEN
    INSERT INTO public.eventos_comerciais (
      ...
      v_action.cliente_id,
      v_action.oportunidade_id,
      v_action.agendamento_id,
      v_action.store_id,
      v_action.seller_id,
      v_event_type::public.crm_evento_tipo,
      now(),
      'central_execucao',
      nullif(trim(coalesce(p_note, '')), ''),
      current_date,
      auth.uid(),
      'central:event:' || p_idempotency_key
    )
    ON CONFLICT (idempotency_key) WHERE idempotency_key IS NOT NULL
    DO NOTHING
...
AS $$
BEGIN
  PERFORM public.central_upsert_appointment_action_internal(
    NEW.id,
    'central:agendamento:' || NEW.id::text
  );
  RETURN NEW;
END;
$$;
...
-- Backfill idempotente dos compromissos comerciais existentes.
INSERT INTO public.execution_actions (
  store_id,
  seller_id,
  source_type,
...
SELECT
  ...
  concat(
    CASE a.tipo::text
      ...
      WHEN 'test_drive' THEN 'Test-drive'
      WHEN 'pos_venda' THEN 'Pos-venda'
      ELSE initcap(replace(a.tipo::text, '_', ' '))
    END,
    ' - ',
  ...
  CASE
    WHEN a.tipo::text IN ('visita', 'negociacao', 'test_drive') THEN 'urgent'
    WHEN a.tipo::text IN ('entrega', 'garantia') THEN 'high'
    ELSE 'medium'
  END,
  CASE a.tipo::text
    WHEN 'visita' THEN 1
  ...
  coalesce(nullif(a.origem_modulo, ''), 'crm'),
  a.id::text,
  'central:agendamento:' || a.id::text,
  a.status::text NOT IN ('compareceu', 'nao_compareceu'),
  true,
  c.nome,
  c.telefone,
  o.veiculo_interesse,
  jsonb_build_object(
    'appointment_status', a.status::text,
    'appointment_type', a.tipo::text,
    'channel', a.canal::text
  ),
  coalesce(a.created_by, a.seller_user_id),
  coalesce(a.updated_by, a.seller_user_id),
