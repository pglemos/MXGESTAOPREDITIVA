-- MX: por decisão do produto (09/07/2026, confirmada após a rodada
-- anterior), o bloqueio de horário 09h31-12h00 (exigindo liberação do
-- gerente) fica DESATIVADO por enquanto — igual à intenção original de
-- 20260709110000, mas mantendo a referência oficial com rollover às 12h00
-- (America/Sao_Paulo) introduzida em 20260709120000/20260709130000, que
-- essa sim é regra correta e deve continuar valendo.
--
-- Ou seja: o vendedor pode enviar/editar o fechamento diário a qualquer
-- horário, sem precisar de liberação. A única trava real de duplicidade
-- continua sendo o ON CONFLICT (upsert) — mesmo registro por vendedor +
-- loja + dia operacional + escopo — e o lock de "já concluído" do lado
-- cliente (fechamentoConcluido em useCheckinPage.ts).
