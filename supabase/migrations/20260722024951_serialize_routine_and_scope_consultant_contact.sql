...
-- The manager route is part of the documented product contract. Keep access
-- bound to the requested store instead of broadening it by role alone.
...
BEGIN
  IF p_store_id IS NULL OR NOT (
    ...
    RAISE EXCEPTION 'Sem permissão para consultar o responsável da loja.' USING ERRCODE = '42501';
  END IF;
  ...
  LEFT JOIN LATERAL (
    ...
    ORDER BY
      CASE linked.assignment_role WHEN 'responsavel' THEN 0 WHEN 'auxiliar' THEN 1 ELSE 2 END,
...
COMMENT ON FUNCTION public.get_owner_consultant_contact(uuid) IS
  'Returns the consulting contact for internal MX, owner, or manager identities scoped to the requested store.';
