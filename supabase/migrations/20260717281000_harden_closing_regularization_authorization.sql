...
BEGIN
  ...
  IF NOT (v_request.seller_id = v_caller OR v_request.requested_by = v_caller) THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Permissão negada.');
  END IF;
...
REVOKE ALL ON FUNCTION public.approve_correction_request(uuid) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.approve_correction_request(uuid) TO authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.reject_correction_request(uuid) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.reject_correction_request(uuid) TO authenticated, service_role;
...
-- DOWN
-- Restore the regularization functions from
-- 20260710130000_canonical_checkin_regularization.sql and the charge function
-- from 20260714140000_cobranca_diaria_idempotente.sql if rollback is required.
-- Restoring the former grants or nullable cancellation predicate reopens known
-- authorization vulnerabilities and requires explicit incident approval.
