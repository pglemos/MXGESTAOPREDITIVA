-- MX: corrige bug introduzido em 20260709120000 — a checagem de liberação
-- usava "minutos desde a meia-noite > 570" sem limite superior, o que
-- bloqueava (exigindo liberação do gerente) QUALQUER envio à tarde/noite de
-- um dia operacional recém-aberto às 12h00 (ex.: 15h29 = 929 min > 570).
--
-- A janela que realmente exige liberação do gerente é 09h31-11h59 (antes do
-- dia operacional rolar às 12h00) — precisa de limite superior também.
...
BEGIN
  ...
  IF v_scope_text = 'daily' THEN
    ...
    -- Janela 09h31-11h59: exige liberação do gerente. Antes das 09h30 está
    -- livre; a partir das 12h00 v_official_reference já rolou para o novo
    -- dia (o reference_date antigo nem chega a bater na checagem acima).
    IF v_minutes_since_midnight > 570 AND v_minutes_since_midnight < 720 THEN
      SELECT EXISTS (
        SELECT 1
