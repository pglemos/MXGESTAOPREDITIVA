-- DOWN para persistência dos dados de troca da Carteira.
-- Restaura a função canônica anterior e remove somente as colunas desta migration.
...
ALTER TABLE public.oportunidades
  DROP COLUMN IF EXISTS veiculo_troca,
  DROP COLUMN IF EXISTS valor_troca;

DROP FUNCTION IF EXISTS public.carteira_salvar_campanha(jsonb, text, uuid, uuid);
DROP FUNCTION IF EXISTS public.carteira_listar_campanhas(uuid, uuid);
DROP FUNCTION IF EXISTS public.carteira_contexto_vendedor(uuid, uuid);
DROP TABLE IF EXISTS public.carteira_campanhas;
