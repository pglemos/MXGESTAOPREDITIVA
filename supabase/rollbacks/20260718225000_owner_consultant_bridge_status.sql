...
-- Reaplica a versão imediatamente anterior da função. O rollback completo
-- da feature é executado por 20260718224500_owner_consultant_bridge.sql.
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    ...
    ORDER BY CASE linked.assignment_role WHEN 'responsavel' THEN 0 WHEN 'auxiliar' THEN 1 ELSE 2 END, linked.created_at ASC
    LIMIT 1
  ) assignment ON true
