-- DOWN: restaura a função auxiliar e o dispatcher introduzidos pela migration
-- 20260721110000_carteira_simulacao_vendedor.sql.
...
BEGIN
  ...
  IF NOT public.eh_area_interna_mx(v_caller) THEN
    RAISE EXCEPTION 'Somente perfis internos MX podem executar uma simulação.' USING ERRCODE = '42501';
  END IF;
  IF NOT EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION 'Vendedor simulado sem vínculo ativo com a loja.' USING ERRCODE = '42501';
  END IF;
  IF v_cliente_id IS NOT NULL AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'Cliente não encontrado ou fora do escopo do vendedor simulado.' USING ERRCODE = '42501';
  END IF;
  IF v_oportunidade_id IS NOT NULL AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'Oportunidade fora do escopo do vendedor simulado.' USING ERRCODE = '42501';
  END IF;
  IF v_agendamento_id IS NOT NULL AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'Agendamento fora do escopo do vendedor simulado.' USING ERRCODE = '42501';
  END IF;
