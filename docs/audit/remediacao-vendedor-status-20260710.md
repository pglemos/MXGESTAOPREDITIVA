...
4. ✅ Provar as transições da Rotina e Mentor com teste automatizado (contador de pendências, atividade vencida sai da lista, reagendamento não duplica competência, venda encerra a atividade certa, garantia não reabre venda). Feito: 6 testes de UI em `CentralExecucao.container.test.tsx` cobrem as 5 transições (contador singular/plural e exclui tratados/de-hoje; "Compareceu" dispara `updateStatus` com o id certo, saída da lista já provada em `agenda-hoje.test.ts`; reagendar usa `updateAgendamento`, nunca `createAgendamento`; venda com duas atividades no dia atualiza a oportunidade certa; garantia cria só o agendamento sem tocar `updateOportunidade`) + 3 testes de contrato puro para `eventoDeCriacaoParaTipo` em `useAgendamentos.test.ts` (garantia→`garantia_registrada`, pós-venda→`pos_venda_realizado`, demais→`agendamento_criado`). **Achado de infra corrigido:** `src/test/setup.ts` não expunha `CustomEvent`/`NodeFilter` do happy-dom — qualquer teste que abrisse um `Modal` (Radix Dialog) dentro de `MemoryRouter` quebrava com `TypeError` no FocusScope; corrigido para todo o repo, não só este teste.
