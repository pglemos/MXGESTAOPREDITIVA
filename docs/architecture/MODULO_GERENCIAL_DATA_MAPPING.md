...
| Metas da loja | regra oficial | `store_meta_rules`, `useStoreGoal` | mês e projeção | nenhum | reutilizar |
| Metas individuais | ranking/meta oficial | hooks de ranking/meta | por vendedor | revisar cobertura | preservar regra oficial |
...
| Plano de ataque/rotina | Central de Execução | `central_execucao_aberturas`, `routine_activity_templates`, `execution_actions` | execução diária | agregação gerencial | criar query composta |
...
| Ranking | score oficial | hooks/features ranking | sem sobrescrever pesos | nenhum | reutilizar |
