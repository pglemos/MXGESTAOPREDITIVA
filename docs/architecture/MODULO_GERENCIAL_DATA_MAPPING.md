...
A especificação textual vence fórmulas, regras, permissões e estados. O Base44 fornece a forma observável quando não houver conflito. As fontes MX abaixo fornecem identidade, autorização e dados persistidos; nenhuma transformação pode converter ausência em zero ou erro em vazio.
