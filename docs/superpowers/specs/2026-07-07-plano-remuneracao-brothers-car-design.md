...
4. Novo componente `bonusCarreira`: lookup de regra `bonus_carreira` por `nivelCarreira`, mesmo padrão de `selecionarRegraMaisRecentePorTipoVeiculo` (reindexado por `nivel_carreira` em vez de `tipo_veiculo`). Fica exposto como campo próprio no resultado (`resultado.bonusCarreira`) pra detalhamento, mas **soma dentro de `bonus`** (`bonus = valorBonusMeta + bonusCarreira`) — assim `total = base + comissao + bonus` continua correto sem tocar em nada que já lê `calculo.bonus` (ex.: `useMinhaRemuneracaoDashboard.ts` tem um `varPay = comissao + bonus` usado no hero de comissão do vendedor).
