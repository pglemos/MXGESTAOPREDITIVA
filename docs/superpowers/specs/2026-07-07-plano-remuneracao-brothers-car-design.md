...
- Bônus individual (item 3 do plano) **não precisa de mudança de motor** — reaproveita o tipo `bonus_meta` existente (`percentual_meta_min = 100`, `valor = 1000`), desde que a meta individual mensal do vendedor (tabela de metas por vendedor, unidade = carros) esteja setada em 8.
...
**`vendedor_perfil`** — coluna nova:
- `nivel_carreira text not null default 'junior'` com CHECK `IN ('junior','pleno','lider')`. Editável só por dono/gerente (não pelo próprio vendedor — reforçar na RLS de update, que hoje é só-o-vendedor; separar esse campo em política própria ou mover a edição pra uma RPC restrita a dono/gerente).
...
2. `comissao_equipe`: quando a regra tem `unidade_meta_min` preenchido, comparar contra `carrosVendidosLoja` em vez de `atingimentoLojaPercentual`. Quando `cumulativo`, somar **todas** as regras cujo limiar foi atingido (hoje pega só a de maior patamar). Quando `valor_por_unidade`, `valor_aplicado = regra.valor × vendasConsideradas` (carros do próprio vendedor). Quando `requer_bonus_individual`, só entra na soma se `regraBonusAplicada` (do cálculo de `bonus_meta`) for não-nulo.
3. Novo componente `comissaoCarreira`: lookup de regra `bonus_carreira` por `nivelCarreira`, mesmo padrão de `selecionarRegraMaisRecentePorTipoVeiculo` (reindexado por `nivel_carreira`). Entra no somatório de `comissao` (ou item próprio em `formulaItens`, a definir na implementação pra ficar claro no extrato do vendedor).
4. `formulaItens` ganha entradas descritivas pros novos componentes (faixas de loja aplicadas, bônus de carreira) — a tela `CalculationDetailsDrawer` já renderiza esse array genericamente.
5. Compatibilidade: regras existentes de outras lojas (`percentual_meta_min`, sem os campos novos) continuam calculando exatamente como hoje — os novos ramos só disparam quando os campos novos estão preenchidos.
...
- `vendedor_perfil.nivel_carreira`:
