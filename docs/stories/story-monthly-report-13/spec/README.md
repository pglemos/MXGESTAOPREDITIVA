...
- **Idempotência**: Uso de logs (PropertiesService no legado, `reprocess_logs` no SaaS) para evitar disparos duplicados para a mesma loja.
...
- **Notificação**: Envio via Resend API para os destinatários configurados em `store_delivery_rules.monthly_recipients`.
