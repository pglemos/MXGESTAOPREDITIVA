...
### Addendum — Teste real em produção (2026-06-26, sessão seguinte)

**A validação manual pendente acima foi executada e reprovou o PASS original.** Autenticação via magic link admin do Supabase (sem uso de senha) para `vendedor@mxgestaopreditiva.com.br` e `gerente@mxgestaopreditiva.com.br` (contas de teste dedicadas, confirmado com o usuário). Clique real em produção revelou **2 bugs P0 que o QA estático não detectou**, ambos corrigidos e reverificados ao vivo na mesma sessão:

1. **As 3 RPCs de liberação nunca funcionaram para um caller autenticado real.** `solicitar_liberacao_fechamento`, `consultar_liberacao_por_token` e `liberar_fechamento_por_token` fixavam `SET search_path = public`, mas `gen_random_bytes`/`digest` (pgcrypto) estão instaladas no schema `extensions` deste projeto. Toda chamada real falhava com `42883 function gen_random_bytes(integer) does not exist`. O teste do QA original usado `SUPABASE_SERVICE_ROLE_KEY` nunca expôs isso porque `auth.uid()` retorna `null` para a service role e a função retorna `'Não autenticado.'` antes de chegar na linha do pgcrypto — um falso-positivo estrutural do método de teste, não do código em si.
2. **A rota `/liberacao-fechamento` não tinha entrada em `ROUTE_ACCESS_RULES`** (`src/lib/auth/routeAccess.ts`), bloqueando por padrão (deny-by-default) até o próprio gerente/dono que deveria acessar o link seguro do WhatsApp — 403 para o público-alvo da funcionalidade.

**Fix:** `supabase/migrations/20260626140000_fix_liberacao_search_path_extensions.sql` (`ALTER FUNCTION ... SET search_path = public, extensions` nas 3 RPCs) + entrada `{ pattern: '/liberacao-fechamento', roles: INTERNAL_AND_LEADERS }` em `routeAccess.ts`. Commit `7f72b0e`, aplicado em produção e revalidado com o mesmo fluxo real: solicitação → página de liberação carregando para `gerente` → liberação efetivada → formulário do vendedor desbloqueado com aviso de penalização → fechamento finalizado com **60% persistido (70% base − 10pp)** → Histórico mostrando o dia como finalizado (`Ajustar`, não mais `Regularizar`).

**Lição estrutural:** QA gate baseado em revisão estática de código + testes unitários/integração com `service_role` pode dar falso PASS em fluxos `SECURITY DEFINER` que dependem de `auth.uid()` real. Qualquer RPC com esse padrão deveria ter ao menos um teste de integração com JWT de usuário real (não service role) antes do gate fechar como PASS.
