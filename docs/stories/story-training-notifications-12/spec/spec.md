...
O EPIC-12 exige inbox operacional por usuário, broadcasts rastreáveis e prescrição tática de treinamentos com base em gargalo. O frontend já assumia esse contrato, mas o live ainda usava o schema reduzido de `notificacoes`.

## Escopo

- Consolidar `notificacoes` como inbox por usuário.
...
- [x] `notificacoes` possui colunas canônicas do app.
...
- `notificacoes` agora contém `recipient_id`, `store_id`, `type`, `priority`, `link`, `read`, `created_at`, `sender_id` e `broadcast_id`.
...
- `docs/stories/story-training-notificacoes-12/spec/spec.md`
- `docs/stories/story-training-notificacoes-12/plan/implementation.yaml`
