...
If the hardened visit policy blocks a legitimate internal consultant flow, create a compensating migration that restores `visitas_consultoria_select` to `public.can_access_consulting_client(client_id)` while preserving `ENABLE ROW LEVEL SECURITY`. Do not disable RLS.
