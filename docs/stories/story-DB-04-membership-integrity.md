...
- [x] Criação de uma Trigger no PostgreSQL (`after_store_delete_cleanup_users`).
- [x] Se um usuário perder sua última afiliação na tabela `memberships` ou `store_sellers`, seu status deve mudar automaticamente para `active = false`.
- [x] Migration versionada garantindo integridade de domínio de exclusão de dados.
